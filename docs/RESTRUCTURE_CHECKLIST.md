...
### 0.2 Core Manifest and Export Macros (`src/shared/core_utils/CoreExports.h`)
...
### 0.3 Core Loader (`src/shared/core_utils/CoreLoader.*`)
...
### 2.1 Shared Registry Layer (`src/shared/core_utils`)
...
- [ ] Create frontend `PluginManager` wrapper.
- [ ] Initialize plugin manager during app startup.
- [ ] Shutdown plugin manager during app teardown.
- [ ] Refactor `ConsoleFactory` to use plugin manager/registry.
- [ ] Refactor `DebuggerFactory` to use plugin manager/registry.
- [ ] Remove runtime dependency on direct concrete backend construction in frontend.
- [ ] Update session/UI availability states from discovered/loaded plugins.

Exit gate:
- [ ] Phase 3 gate: frontend runtime creation path is plugin-only.
