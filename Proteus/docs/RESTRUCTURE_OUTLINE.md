...
- Core export macros and `CoreManifest` exist in `src/shared/plugin_utils/CoreExports.h`.
- A cross-platform `CoreLoader` implementation exists in `src/shared/plugin_utils/CoreLoader.*`.
...
Work:
- Add `PluginRegistry` in `src/shared/plugin_utils`.
